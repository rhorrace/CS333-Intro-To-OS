...
// Begin additional test
  // Test priority of nonexistent PID
  printf(1, "  Testing that a priority of a nonexistent PID can't be retrieved.\n");
  pid = 32767;
  int prio = getpriority(pid);
  if(prio != -1) {
    printf(2, "  Attempted to retrieve nonexistent PID's priority, which should fail.\n");
    printf(2, "This should have failed, leading to the test passing.\n");
    printf(2, "  **** TEST FAILED ****\n\n");
  } else {
    printf(1, "  **** TEST PASSED ****\n\n\n");
  }
